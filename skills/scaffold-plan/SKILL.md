...
Read: `state.md` (a `## Next` already carrying a concrete instruction from `checkpoint` — "re-finalize `…/04-y.md`, its `## Targets` predate [[0007-…]]" — is the agreed direction; confirm it in Phase 2 and act on it); `roadmap.md`; `project.md`; `architecture.md`; `glossary.md` if present (write in the project's words; entries are `checkpoint`'s to propose). Then the **active milestone** — the one `## Next` names; folder order is only a fallback when Next is silent: its `milestone.md`, the phase plan Next points at, its `spec/` (follow a pointer, never crack open the target's internals), relevant `knowledge/`. Scan `decisions/` and `investigations/` by filename; read what is directly relevant. Look into the code as far as the plan at hand warrants — deep for a phase about to run, lightly for one far down the road; your judgment. Delegate any read that spans several files to fresh read-only subagents on the fastest cheap model available; they return what to read and why (paths, headings — no summary, no verdict), and the reading and judgment are yours.

...
**Auto-finalize when next up.** If one of the plans you just authored is the one `state.md`'s `## Next` will point at, offer to finalize it in the same act — "I'll finalize 07-slug now so it's ready to run?" — and on his yes run the Finalize pass below without a re-invocation. On a pivot (inserting a phase is one), run Phase 6's sweep first: it rewrites the very siblings the neighbour check reads. **Only that one plan.** Every sibling stays a draft: finalizing stamps a plan against code that will have moved by the time it runs, so a batch-finalized set arrives at `go` stale.
...
Steps 1–6 edit the plan file in place; `## Targets` and `state.md` are written only at step 7, on confirmation, so a pass that stops or is abandoned leaves a draft on disk. On a *re*-finalize, delete the existing `## Targets` at step 1 — a stale stamp on a rewritten scope reads as *final & fresh* to `go`.
...
2. **Tighten Scope/Approach** against what the code actually is, and **ensure `## Acceptance` is user-verifiable** — an observable outcome, not "tests pass".
3. **Neighbour check.** List `.scaffold/milestones/NN-slug/phases/` and read the milestone's `milestone.md` `## Phases` (this pass skips Phase 1's triage, so nothing else has put them in front of you). The siblings are every other plan in the folder whose entry is unticked or missing — note a missing entry for `/scaffold-checkpoint` to add. Read each sibling's `## Objective`, `## Scope` and `## Approach`, those sections only: no code, no other milestone. `## Approach` is in the set because a written seam lives there; without it a resolved overlap re-reports on every re-finalize.

   Per scope item of this plan, one test: **if that sibling executed first, exactly as written, would this item still need doing in full?** Yes → not a finding, even when both touch the same file. No, or only partly → a hit — but only if you can name the one artifact (a file, table, test, document, dataset) both would leave in the same end state. The other route to a hit is an **unresolved forward pointer**: prose in either plan naming an unexecuted sibling without saying who owns the work. Never a finding: both plans pointing at the same rule doc, this plan building on what a sibling produces, a sequencing preference, a pointer that already names an owner. If a sibling is too vague to answer the test, put the question to the user at step 6 rather than clearing or admitting the item yourself.

   **Every hit resolves by an edit before finalize, and every resolution surfaces at step 6.** The legal moves: narrow this item to the remainder the sibling leaves and say so in `## Approach`; take the whole item here and drop it from a draft sibling; drop it here when the sibling leaves it wholly done; or write the owner into this plan's `## Approach` ("`08-splits` item 6 owns adding it; this phase assumes it exists"). On a whole hit the item goes to whichever phase executes first (by default the lower-numbered plan), and after any drop check that the losing plan still stands on its own at the point it runs. Never drop an item a sibling only partly covers — the remainder then gets built zero times — and never resolve a pointer by deleting it. The seam sentence always lands in this plan, so the check has one writer. **A finalized sibling (one carrying `## Targets`) is never edited in place** — its stamp would then lie about what `go` builds. If a hit can only be resolved by changing one, hard-stop: this plan stays a draft, name the sibling and the item, and run this pass again from step 1 once that sibling has been re-finalized.

   Zero hits is the normal result. If the hits together say the phase cut itself is wrong, say so at step 6 and propose a re-cut alongside the resolutions.
4. **Apply the stranger test.** Ask: could a competent builder who has never seen this
   ...
   a different class from what step 1's code research finds. Where the answer names a rule that already lives in `knowledge/` or `decisions/`, the fix is a `## Governed by` entry (step 5) plus a line in `## Approach` on how it applies — never a pasted copy.
5. **Write `## Governed by`.** List every file in `.scaffold/knowledge/` and `.scaffold/decisions/` with its opening rule line; the section is chosen from that listing, never from memory. One test per candidate: **if you built this phase in violation of this document's rule, would the phase be wrong?** Yes → list it, naming the rule (not the doc's topic) in a few words; if you cannot name the rule, it does not belong. A `decisions/` file is a candidate only while its `Status:` is `Accepted`: a `Superseded` one is replaced by its successor if that binds; a `Proposed` one that would bind stops finalize until the ADR gate (Phase 3) resolves it. Keep the list tight — `go` reads every entry in full before building.

   Entries are repo-relative paths, one per line, in a section placed after `## Acceptance`:
   ...
   If nothing binds, write no section and put this line in `## Approach` instead — fixed wording, because the check is a grep:

   > Governed by: none — no `knowledge/` or `decisions/` document constrains this phase.

   A finalized plan carries exactly one of the two forms. On a re-finalize, replace the section rather than appending, and delete whichever form no longer applies — a bare heading is malformed. Point, don't copy: `## Approach` says how a rule applies to this phase; the pointer supplies the rule.
6. Present the approach in plain terms and confirm in dialogue — including each neighbour-check resolution and any question it raised. The user is an architect who doesn't read the plan or the code — so surface the approach as a plain-language conversation ("here's how I'll do it: …"), not "read this doc". **Wait for his confirmation.** This is the approval seam; `go` executes afterward without re-approving.
7. On confirmation, apply whatever the dialogue changed, then **write `## Targets`** — last in the file, after `## Governed by` — one entry per file/interface the phase touches, stamped with the current commit: `git rev-parse --short HEAD`, written as `_as of <sha>_` under the heading. This is the grounding evidence that makes the plan auditable and gives `go` its staleness backstop. Then set `state.md`'s Active focus + `## Next` so a resuming session knows the plan is final & fresh.
...
A plan authored earlier in *this* invocation is swept like any other — which is why Phase 5's auto-finalize offer waits for this sweep.
...
This sweep hunts **conflict** and fires only when something moved; the finalize neighbour check hunts **duplication**, which no pivot surfaces. Neither substitutes for the other.
...
the `## Governed by` entries written and how each neighbour-check hit was resolved. Then:

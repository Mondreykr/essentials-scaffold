...
- **Plan state?** From the active plan's `## Targets`: none → **draft**; present and the stamp holds → **final & fresh**; stamp doesn't hold → **stale**. The stamp holds when `git merge-base --is-ancestor <sha> HEAD` succeeds *and* every path in `git diff --name-only <sha> --` is a `## Targets` path entry or under `.scaffold/` (a checkpoint commit is not staleness; untracked files don't count). No git → report *final, freshness unverifiable (no git)*.

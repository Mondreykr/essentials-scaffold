...
4. **Ceremony scales with the user.** You decide how much structure you want per
   ...
   equally.
5. **Don't tell Claude what it already knows.** Skills and rules only instruct
   behaviors Claude wouldn't do by default.
6. **Content-derived state, no enums.** What's active, what's done, and what mode a
...
Every document type has one canonical **format contract** in `contracts/` — the exact form (required sections, skeleton, rules, anti-patterns). Skills carry an inline paraphrase at the altitude they need; only `/scaffold-audit` bundles the contracts verbatim (`references/`, regenerated by `scripts/sync-contracts.sh`, drift-guarded by `--check`).
...
never stored** (a stored band would be a driftable enum — Principle 6).
...
- **Freshness — the stamp holds until something moves that the plan did not declare.** The question is *not* "has the repo moved?" A phase that spans a `/clear` moves the repo by construction, and so does `checkpoint` committing `.scaffold/`. Asking the broad question makes a harmless save and a stranger's commit **indistinguishable** — both merely differ from HEAD — so the gate fires on every resume, and a gate you pass without reading is not a gate. The test is deterministic and judges nothing:
  ...
  Both exemptions are principled, not conveniences. **Target files** already move under `go` item-by-item within one session with no re-check — the code moving is what executing *is* — so letting them move across a session boundary is exactly as safe, and erasing that boundary is what scaffold exists to do. **`.scaffold/` files** belong to the **plan-set drift** obligation: doc drift is defended by `plan`'s pivot sweep and `checkpoint`'s coherence sweep, never by this check. A third exemption follows from the repair licence: **a project file touched *only* by `checkpoint`/`reconcile` commits since the stamp** — refusing a resume because the last save tidied a dead import is the same false refusal the first two exemptions exist to prevent. *Only*, because a file a checkpoint tidied and a stranger then edited has moved in the sense the gate exists to catch. Untracked files never trip the gate.
...
- **Staleness obligations — three.** Because plans *persist*, they can go stale three ways, each with one defense: **finalize→execute drift** (code moves before `go` runs) → the *Freshness* gate above; **plan-set drift** (phases reordered/cut, a plan premised on a since-superseded decision) → `plan`'s pivot sweep over all *unexecuted* plans, drafts included (a draft on a superseded ADR still breaks the ADR gate), backstopped by `checkpoint`'s coherence sweep on a finalized plan vs a later decision; **plan-set coherence** → the bullet below.
...
- **Milestone lifecycle.** Active = wherever `state.md` Next points (not folder
  order). On close, `checkpoint` graduates the spec's enduring rules into `knowledge/` (reconciling and retiring contradicted docs, surfaced for Adam), flips the `roadmap.md` line to `[done]`, `git mv`s the folder whole to `milestones/archived/NN-slug/`, stamps `archived: YYYY-MM-DD` into that folder's `milestone.md` frontmatter only (the path already says what the spec and plans are), repoints the roadmap link and `state.md` `## Next` in the same act, and prints the old and new paths — references from outside `.scaffold/` break on the move and scaffold sweeps none of them. Any remaining `## Deferred` items are resolved, promoted, or dropped at close — they retire with the milestone, never silently graveyarded.
...
| The contract that scoped a milestone | `milestones/NN-slug/spec/` (the spec, or a pointer to a shared/external one — which stays whole and is maintained in place until the milestone closes). Once closed: `milestones/archived/NN-slug/spec/` — a record of what was built, never a statement of current behaviour |
...
each a self-contained artifact that carries the format guidance it needs, written in.
...
- **`decisions/` is propose-only.** Skills may draft an ADR and stop; **Adam approves**
  before anything is written, superseded, or pruned.
...
doc is surfaced and proposed via `plan`/`checkpoint` (Adam-gated).
...
- **draft** → stop and route to `/scaffold-plan --final` or freeform work; `go` has no research/propose step of its own.
- **stale** → stop, **name the offending files**, route to `/scaffold-plan --final`.
...
- **Always runs the light sweep** (*The two-tier audit model*) over *all* living docs, not just the touched ones: structural — required sections in order, frontmatter, no catch-all / append-log, no `project.md` checkbox (Law 2); coherence — cross-references resolve, Law-1/Law-2, duplication, plan-vs-decision staleness, `## Next` resolves, stale dates.
...
- **Runs the milestone-close motion** (*Execution model → Milestone lifecycle*).
...
- **Disposes of every finding** (the four dispositions: *Skills → structural boundaries*). The on-disk routes: an undecided call → `## Open Questions`; a real obstacle → `## Blockers`; work clearing the admission bar → `## Deferred` / `## Backlog`; work needing authoring → the concrete instruction written into `## Next`, phrased so a cold session can act on it.
...
Audit ends by naming the disposing skill for each finding and, when no `checkpoint` follows at once, says plainly that the findings live only in this transcript until one does.
...
`update` is omitted — it pulls skill files and touches no `.scaffold/` content.
...
Each skill states what it does NOT do (the **Boundary** paragraph every skill opens with). `integrate` is the thinnest skill, and that is deliberate: it owns the *ingest-vs-author*
...
Bulk reading belongs in a fresh read-only subagent, so the session's judgment stays uncrowded by file dumps; judgment, and anything needing the session's own memory (what happened, what the user agreed to), stays in the session. Two kinds of subagent job, and they differ in lane and model. A **locating** job (plan's code research) returns what to read and why — paths and headings, no summary, no verdict — because a quoted snippet anchors the orchestrator and smuggles interpretation in as evidence; the reading and the judging are the session's, so the job runs on the fastest cheap model available. A **verifying** job (go's scope check, audit's passes) exists for an independent verdict and runs on the session's model. The skills state the goal and leave the how to Claude: `plan` decides how far into the code to look — deep for a plan about to run, lightly for one far down the road — and marks independent scope items so `go` can build them in parallel; `go` decides whether to fan out. Nothing prescribes agent count or step order.

...
| `/scaffold-checkpoint` | Verifies work, updates scaffold docs, runs a light structural + coherence sweep, **fixes what it finds**, commits. Every finding is fixed, ruled on by you and applied, or routed to a section on disk — never just mentioned. Auto-detects a no-work run and just sweeps. | End of every session, or whenever you want to save |
...
Three boundaries hold across the set: **`go` writes code, never scaffold docs** (all scaffold write-back is `plan`/`checkpoint`'s job — with one bounded exception: `checkpoint` may repair the small residue of the session whose diff it is already holding, shown to you in the pre-commit review); **`decisions/` is propose-only** — a skill may draft an ADR but stops for your approval before writing it; and **findings terminate at `checkpoint`** — whatever the sweep, `audit`, or `go`'s scope check turns up gets fixed, ruled on, or written to a section on disk before the commit, never left in the transcript.
...
A phase plan (`phases/NN-slug.md`) carries one phase's Objective / Scope / Approach / Acceptance. Plans are authored up front (predetermined milestone, from a spec) or just-in-time by `plan` (emergent milestone). A plan starts as a **draft**; `plan --final` finalizes it — adding a `## Targets` section (the files it touches, stamped `as of <sha>`) once it's validated against current code. `go` executes from a plan's `## Scope`, but only when the plan is finalized and its stamp still **holds** — meaning the stamped commit is still in this branch's history and nothing has moved since except the files the plan declared in `## Targets` and scaffold's own `.scaffold/` docs. The question is "did anything move that this plan didn't declare?", not "has the repo moved?" — so pausing mid-phase and resuming next session keeps a plan fresh, while someone else's commit makes it stale and `go` names the files.
